...
async function run() {
  ...
  const r1 = await handler({ httpMethod: 'OPTIONS', body: null });
  ...
  const r2 = await handler({ httpMethod: 'GET', body: null });
  ...
  const r3 = await handler({ httpMethod: 'POST', body: JSON.stringify({ subject:'test', message:'hallo' }) });
  ...
  const r4 = await handler({ httpMethod: 'POST', body: 'GEEN_JSON' });
  ...
  console.log('\n✓ Alle 4 tests geslaagd. Module laadt correct, JSON responses kloppen.');
}
